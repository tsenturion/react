...
// ("Docker вообще вне браузера..."), поэтому размер текста здесь подстраивается
// от содержимого, а не жёстко задаётся одной типографикой.
...
export function ProjectStudy({
  ...
  snippets: { label: string; note: string; code: string }[];
}) {
  return (
    <div className="space-y-6">
      <p className="text-sm leading-6 text-slate-600">
        Ниже идут не абстрактные примеры, а файлы и фрагменты именно этого проекта.
        Смотри, как тема страницы выражена в текущем коде: через компоненты, состояние,
